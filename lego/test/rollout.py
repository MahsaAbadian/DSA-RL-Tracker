...
Loads tracker module movement weights and uses a standalone stop module
...
# Add repo root + Lego modules to sys.path for imports
_script_dir = os.path.dirname(os.path.abspath(__file__))
_lego_root = os.path.dirname(_script_dir)
_repo_root = os.path.dirname(_lego_root)
for _path in (_repo_root, _lego_root):
    if _path not in sys.path:
        sys.path.insert(0, _path)

# Imports from tracker module and stop module
from tracker_module.models_deeper import ActorOnly
...
CROP = 33
DEFAULT_MOVEMENT_WEIGHTS = os.path.join(_script_dir, "final_weights", "tracker_weights", "experiment4_20260121.pth")
DEFAULT_STOP_WEIGHTS = os.path.join(_script_dir, "final_weights", "stop_weights", "stop_detector_v1.pth")
...
class InferenceEnv:
    ...
    def step(self, a_idx, stop=False):
        self.steps += 1
        if stop:
...
def main():
    ...
    parser.add_argument("--movement_weights", type=str, default=DEFAULT_MOVEMENT_WEIGHTS,
                        help="Path to 8-action movement weights (.pth)")
    parser.add_argument("--stop_weights", type=str, default=DEFAULT_STOP_WEIGHTS,
                        help="Path to standalone stop module weights (.pth)")
    parser.add_argument("--max_steps", type=int, default=1000, help="Maximum steps before stopping")
    parser.add_argument("--stop_threshold", type=float, default=0.2, help="Stop probability threshold")
    parser.add_argument("--stop_logit_bias", type=float, default=1.5,
                        help="Additive bias to stop logit (higher = more likely to stop)")
    parser.add_argument("--min_steps_before_stop", type=int, default=10,
                        help="Prevent stop module from triggering before this many steps")
    parser.add_argument("--stop_debug_every", type=int, default=0,
                        help="Print stop probability every N steps (0 = off)")
    ...
    if not os.path.exists(args.movement_weights):
        print(f"Error: movement weights not found: {args.movement_weights}")
        return
    if not os.path.exists(args.stop_weights):
        print(f"Error: stop weights not found: {args.stop_weights}")
        return

    K = 16
    movement_model = ActorOnly(n_actions=N_MOVEMENT_ACTIONS, K=K).to(DEVICE)
    stop_model = StandaloneStopDetector().to(DEVICE)

    try:
        movement_state = torch.load(args.movement_weights, map_location=DEVICE)
        if isinstance(movement_state, dict) and "state_dict" in movement_state:
            movement_state = movement_state["state_dict"]
        if isinstance(movement_state, dict):
            actor_state = {k: v for k, v in movement_state.items() if k.startswith("actor_")}
            if actor_state:
                movement_state = actor_state
    ...
    try:
        ...
        missing_keys, unexpected_keys = stop_model.load_state_dict(stop_state, strict=False)
        if missing_keys:
            print(f"⚠️  Stop model missing keys: {len(missing_keys)} (using random init)")
        if unexpected_keys:
            print(f"⚠️  Stop model unexpected keys: {len(unexpected_keys)}")
        
        # Test the model with a dummy input to check if it's working
        with torch.no_grad():
            dummy_input = torch.randn(1, 2, 33, 33, device=DEVICE)
            dummy_logit = stop_model(dummy_input)
            print(f"✓ Loaded stop weights from: {args.stop_weights}")
            print(f"   Test logit (random input): {dummy_logit.item():.3f} -> prob: {torch.sigmoid(dummy_logit).item():.4f}")
    except Exception as e:
    ...
    stop_debug_every = int(args.stop_debug_every)
    ...
    while not done:
        ...
        with torch.no_grad():
            movement_logits, _hc = movement_model(obs_t, A_t)
            ...
            if stop_debug_every > 0 and env.steps % stop_debug_every == 0:
                stop_logit_raw = float((stop_logit - stop_logit_bias).view(-1)[0].item())
                stop_logit_with_bias = float(stop_logit.view(-1)[0].item())
                img_min = float(obs_t[:, 0:1].min().item())
                img_max = float(obs_t[:, 0:1].max().item())
                path_sum = float(obs_t[:, 3:4].sum().item())
                print(
                    f"[step {env.steps:04d}] stop_logit_raw={stop_logit_raw:.3f} "
                    f"(+bias={stop_logit_bias:.1f})={stop_logit_with_bias:.3f} "
                    f"stop_prob={stop_prob.item():.4f} img=[{img_min:.3f},{img_max:.3f}] "
                    f"path_sum={path_sum:.1f}"
                )

            if env.steps >= min_steps_before_stop and stop_prob.item() > args.stop_threshold:
                action = None
                stop_now = True
            else:
        ...
        done, reason = env.step(action, stop=stop_now)

        new_onehot = np.zeros(N_MOVEMENT_ACTIONS)
        if action is not None and action < N_MOVEMENT_ACTIONS:
