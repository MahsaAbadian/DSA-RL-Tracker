...
# ---------- REWARD CONFIG LOADING ----------
def default_reward_config():
    """Return the default reward configuration (matches original Experiment1 behavior)."""
    return {
        "stop": {
            "bonus_non_strict": 20.0,
            "bonus_strict": 30.0,
            "incorrect_penalty": -2.0,
            "reach_shaping_strict": 5.0
        },
        "distance_reward": {
            "epsilon": 1e-6,
            "log_clip_min": -2.0,
        ...
        "bootstrap": {
            "enabled": True,
            "max_steps": 5,
            "multiplier_start": 3.0,
            "multiplier_decay": 0.4,
            "progress_reward_scale": 1.5,
            "getting_closer_scale": 0.5,
            "progress_penalty_floor": -0.5
        },
        "direction_hint": {
            "scale": 0.3
        }
...
def load_reward_config(config_path=None):
    ...
    if os.path.exists(config_path):
        with open(config_path, "r") as f:
            cfg = json.load(f)
        print(f"✓ Loaded reward configuration from: {config_path}")
        return cfg, config_path
    else:
        print(f"⚠️  Reward config not found: {config_path}, using defaults")
        return default_reward_config(), None
...
class CurveEnvUnified:
    def __init__(self, h=128, w=128, max_steps=200, base_seed=BASE_SEED, stage_id=1, curve_config=None, reward_config=None):
        self.h, self.w = h, w
        self.max_steps = max_steps
        self.base_seed = base_seed
        self.current_episode = 0
        self.curve_config = curve_config or {}
        self.reward_cfg = reward_config or default_reward_config()
    ...
    def step(self, a_idx: int):
        ...
        if a_idx == ACTION_STOP_IDX:
            if dist_to_end < 5.0:
                stop_bonus = self.reward_cfg["stop"]["bonus_strict"] if self.stage_config['strict_stop'] else self.reward_cfg["stop"]["bonus_non_strict"]
                return self.obs(), stop_bonus, True, {"reached_end": True, "stopped_correctly": True}
            else:
                return self.obs(), self.reward_cfg["stop"]["incorrect_penalty"], False, {"reached_end": False, "stopped_correctly": False}
        ...
        eps = self.reward_cfg["distance_reward"]["epsilon"]
        clip_lo = self.reward_cfg["distance_reward"]["log_clip_min"]
        clip_hi = self.reward_cfg["distance_reward"]["log_clip_max"]

        if L_t < self.L_prev:
            r = np.log(eps + dist_diff)
        else:
            r = -np.log(eps + dist_diff)
        ...
        if self.is_at_start and self.initial_steps <= 5 and len(self.ep.gt_poly) > 1:
            ...
            if curve_dir_norm > 1e-6:
                ...
                if action_dir_norm > 1e-6:
                    ...
                    direction_bonus = direction_alignment * (3.0 - self.initial_steps * 0.4) * self.reward_cfg["direction_hint"]["scale"]
                    r += direction_bonus

        if progress_delta > 0:
            r += precision_score * self.reward_cfg["progress"]["precision_scale"]
        elif progress_delta <= 0:
            r += self.reward_cfg["progress"]["no_progress_penalty"]
        ...
        if self.reward_cfg["bootstrap"]["enabled"] and self.is_at_start and self.initial_steps <= self.reward_cfg["bootstrap"]["max_steps"]:
            bootstrap_multiplier = max(
                1.0,
                self.reward_cfg["bootstrap"]["multiplier_start"] - self.initial_steps * self.reward_cfg["bootstrap"]["multiplier_decay"]
            )
            if progress_delta > 0:
                r += precision_score * bootstrap_multiplier * self.reward_cfg["bootstrap"]["progress_reward_scale"]
            elif L_t < self.L_prev:
                r += abs(r) * bootstrap_multiplier * self.reward_cfg["bootstrap"]["getting_closer_scale"]
            if progress_delta < 0:
                r = max(r, self.reward_cfg["bootstrap"]["progress_penalty_floor"])
        ...
        if self.stage_config['stage_id'] >= 2:
            ...
            if norm_gt > 1e-6 and norm_act > 1e-6:
                ...
                if cos_sim > 0: r += cos_sim * self.reward_cfg["progress"]["cos_alignment_scale"]

        if self.prev_action != -1 and self.prev_action != a_idx:
            r += self.reward_cfg["step_penalty"]["switch_penalty"]
        self.prev_action = a_idx
        r += self.reward_cfg["step_penalty"]["base"]
        ...
        off_track_limit = self.reward_cfg["offtrack"]["limit_stage1"] if self.stage_config['stage_id'] == 1 else self.reward_cfg["offtrack"]["limit_other"]
        off_track = L_t > off_track_limit
        
        if off_track:
            r += self.reward_cfg["offtrack"]["penalty"]
            done = True
        ...
        if self.stage_config['strict_stop'] is False:
            if reached_end:
                r += self.reward_cfg["stop"]["bonus_non_strict"]
                done = True
        else:
            if reached_end:
                r += self.reward_cfg["stop"]["reach_shaping_strict"]
...
def run_unified_training(run_dir, base_seed=BASE_SEED, clean_previous=False, experiment_name=None, resume_from=None, curve_config_path=None, reward_config_path=None):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(script_dir)  # Experiment1 directory
    ...
    reward_config, actual_reward_config_path = load_reward_config(reward_config_path)
    ...
    if resume_from is not None:
        ...
        # Load reward config - try run directory first, then saved path
        saved_reward_config_path = saved_config.get('reward_config_path', None)
        reward_config_in_run = os.path.join(run_dir, "reward_config.json")
        if os.path.exists(reward_config_in_run):
            reward_config, actual_reward_config_path = load_reward_config(reward_config_in_run)
            print(f"✓ Loaded reward config from run directory: {actual_reward_config_path}")
        elif saved_reward_config_path and os.path.exists(saved_reward_config_path):
            reward_config, actual_reward_config_path = load_reward_config(saved_reward_config_path)
            print(f"✓ Loaded reward config from saved path: {actual_reward_config_path}")
        else:
            print(f"⚠️  Using reward config from command line/default")
            # Keep the reward_config loaded earlier
    ...
    # Copy reward configuration file to run directory for reproducibility
    run_reward_config_path = os.path.join(run_dir, "reward_config.json")
    if resume_from is None:
        if actual_reward_config_path and os.path.exists(actual_reward_config_path):
            shutil.copy2(actual_reward_config_path, run_reward_config_path)
            print(f"✓ Copied reward configuration to: {run_reward_config_path}")
            stored_reward_config_path = "reward_config.json"
        else:
            stored_reward_config_path = reward_config_path if reward_config_path else None
    else:
        if os.path.exists(run_reward_config_path):
            print(f"✓ Using reward configuration from run directory: {run_reward_config_path}")
            stored_reward_config_path = "reward_config.json"
        else:
            if actual_reward_config_path and os.path.exists(actual_reward_config_path):
                shutil.copy2(actual_reward_config_path, run_reward_config_path)
                ...
                if saved_config:
                    stored_reward_config_path = saved_config.get('reward_config_path', None)
                else:
                    stored_reward_config_path = None
    ...
    training_config = {
        ...
        "reward_config_path": stored_reward_config_path,
        "image_height": img_h,
        "image_width": img_w,
        "stages": []
    }
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        env = CurveEnvUnified(h=img_h, w=img_w, base_seed=base_seed, stage_id=stage['config']['stage_id'], curve_config=curve_config, reward_config=reward_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reward_config", type=str, default=None,
                        help="Path to reward configuration JSON file (default: config/reward_config.json)")
    ...
    run_unified_training(args.run_dir, args.base_seed, args.clean_previous, 
                        args.experiment_name, args.resume_from, args.curve_config, args.reward_config)
